import {getprojectId, getFireBaseProject, setShowProjectCalender,setProject, setShowProject,setProjectName } from "../actions";
  ...
  return (
    <ul>
        <div>
          {getproject.map((data) => (
            <li onClick={() => {
              dispatch(setShowProjectCalender(!setProjectToggle.setshowprojectcalender));
              dispatch(getprojectId(setProjectToggle.projectId=data.projectId));
            }}>{data.name}</li>
          ))}
        </div>
